...
//
//   node build.mjs          aggressive build: one minified index.html with the CSS and the JS inlined, and
//                           the favicon inlined as a data: URI. The browser mockup is too big to inline
//                           comfortably, so it stays a file, minified into media/landing/.
...
if (plain) {
  // ---------------------------------------------------------------- plain
  // normalize so that "../addon/ui/icons/logo.svg" becomes "addon/ui/icons/logo.svg" at the repo root
  html = mapUrls(html, (url) => path.posix.normalize("landing/" + url));
  after = bytes(html);
  await mkdir(outDir, { recursive: true });
  await writeFile(path.join(outDir, "index.html"), html, "utf8");
  // a custom --out has no landing/ next to it: bring the assets along
  if (path.resolve(outDir, "landing") !== here) {
    for (const f of ["landing.css", "landing.js", "mockup.svg"])
      await cp(path.join(here, f), path.join(outDir, "landing", f), { recursive: true });
    log.push(`  + landing/{landing.css,landing.js,mockup.svg} copied to ${path.join(outDir, "landing")}`);
    log.push(`  note: the favicon points at addon/ui/icons/logo.svg, which is not copied`);
  }
} else {
  // ---------------------------------------------------------------- aggressive
  ...
  // Full default preset, except that the viewBox is kept (the mockup scales with it). svgo drops the
  // mockup's <title>; the page describes it with the alt text on the <img> instead.
